...
class OpReadEntry implements ReadEntriesCallback {
    ...
    private void internalReadEntriesComplete(List<Entry> returnedEntries) {
        ...
        log.debug()
                .attr("managedLedger", cursor.ledger.getName()).attr("cursor", cursor.getName())
                .attr("batchSize", returnedEntries.size()).attr("cumulativeSize", entries.size())
    ...
    private void internalReadEntriesFailed(ManagedLedgerException exception, Object ctx) {
        ...
        if (!entries.isEmpty()) {
            ...
            log.warn()
                    .attr("managedLedger", cursor.ledger.getName()).attr("cursor", cursor.getName())
                    .attr("readPosition", readPosition).exceptionMessage(exception)
            ...
            if (!(exception instanceof TooManyRequestsException)) {
                log.warn()
                        .attr("managedLedger", cursor.ledger.getName()).attr("cursor", cursor.getName())
                        .attr("readPosition", readPosition).exception(exception)
                        .log("Read failed from ledger");
            } else {
                log.debug()
                        .attr("managedLedger", cursor.ledger.getName()).attr("cursor", cursor.getName())
    ...
    private void complete(Object ctx) {
        cursor.ledger.getExecutor().execute(() -> {
            try {
                ...
                log.error().attr("op", this).attr("lastPosition", lastEntryPosition()).exception(throwable)
