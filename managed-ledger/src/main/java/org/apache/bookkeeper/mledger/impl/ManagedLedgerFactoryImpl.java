...
public class ManagedLedgerFactoryImpl implements ManagedLedgerFactory {
    ...
    public void estimateUnloadedTopicBacklog(PersistentOfflineTopicStats offlineTopicStats,
        ...
        log.debug().attr("managedLedger", managedLedgerName).attr("numberOfEntries", numberOfEntries)
                .attr("totalSize", totalSize).log("Total number of entries and size");
                    ...
                    public void operationComplete(ManagedLedgerInfo mlInfo, Stat stat) {
                        ...
                        if (!ledgers.isEmpty()) {
                            ...
                            AsyncCallback.OpenCallback opencb = (rc, lh, ctx1) -> {
                                log.debug().attr("managedLedger", managedLedgerName).attr("ledgerId", id)
                                        .attr("result", BKException.getMessage(rc)).log("Opened ledger");
                            ...
                            getBookKeeper()
                                    .thenAccept(bk -> {
                                        ...
                                        log.warn().attr("managedLedger", managedLedgerName).attr("ledgerId", id)
                                                .exception(ex).log("Failed to open ledger");
    ...
    public void calculateCursorBacklogs(final TopicName topicName,
        ...
        log.debug().attr("managedLedger", managedLedgerName).attr("lastLedgerPosition", lastLedgerPosition)
        ...
        store.getCursors(managedLedgerName, new MetaStore.MetaStoreCallback<List<String>>() {
            ...
            public void operationComplete(List<String> cursors, Stat v) {
                ...
                log.debug().attr("managedLedger", managedLedgerName).attr("numCursors", cursors.size())
                        .log("Found cursors");
                ...
                for (final String cursorName : cursors) {
                    ...
                    AsyncCallback.OpenCallback cursorLedgerOpenCb = (rc, lh, ctx1) -> {
                        ...
                        log.debug().attr("managedLedger", managedLedgerName).attr("ledgerId", ledgerId)
                                .attr("cursor", cursorName).attr("rc", rc).log("Opened cursor ledger");
                        if (rc != BKException.Code.OK) {
                            log.warn().attr("managedLedger", managedLedgerName).attr("ledgerId", ledgerId)
                                    .attr("cursor", cursorName).attr("result", BKException.getMessage(rc))
                        ...
                        log.debug().attr("managedLedger", managedLedgerName).attr("cursor", cursorName)
                                .attr("lac", lac).attr("ledgerId", ledgerId).log("Cursor LAC read from ledger");
                        ...
                        if (lac == LedgerHandle.INVALID_ENTRY_ID) {
                            ...
                            log.info().attr("managedLedger", managedLedgerName).attr("cursor", cursorName)
                                    .attr("lac", lac).attr("ledgerId", ledgerId).log("Cursor LAC read from ledger");
                            cursorCounter.countDown();
                            return;
                        }
                                    ...
                                    if (rc != BKException.Code.OK) {
                                        ...
                                        try {
                                            ...
                                            log.warn().attr("managedLedger", managedLedgerName)
                                                    .attr("ledgerId", ledgerId).attr("cursor", cursorName).exception(e)
                                        ...
                                        Range<Position> range = Range.openClosed(lastAckedMessagePosition,
                                                lastLedgerPosition);
                                        log.debug().attr("managedLedger", managedLedgerName).attr("cursor", cursorName)
                                                .attr("range", range).log("Calculating backlog for cursor");
                    ...
                    store.asyncGetCursorInfo(managedLedgerName, cursorName,
                            new MetaStore.MetaStoreCallback<ManagedCursorInfo>() {
                                ...
                                @Override
                                public void operationFailed(ManagedLedgerException.MetaStoreException e) {
                                    log.warn().attr("managedLedger", managedLedgerName).attr("cursor", cursorName)
                                            .exception(e).log("Unable to obtain cursor ledger for cursor");
                                    cursorCounter.countDown();
                                }
                            });
        ...
        if (accurate && ledgerRetryMap.size() > 0) {
            ledgerRetryMap.forEach((cursorName, ledgerId) -> {
                log.debug().attr("cursor", cursorName).attr("ledgerId", ledgerId)
                ...
                if (lastAckedMessagePosition == null) {
                    log.warn().attr("managedLedger", managedLedgerName).attr("cursor", cursorName)
                            .attr("ledgerId", ledgerId).log("Unable to determine cursor position");
                } else {
                    log.debug().attr("managedLedger", managedLedgerName).attr("cursor", cursorName)
                            .attr("ledgerId", ledgerId).attr("position", lastAckedMessagePosition)
    ...
    private Position tryGetMDPosition(BookKeeper bookKeeper, long ledgerId, String cursorName) {
        ...
        try {
            ...
            for (LedgerEntry ledgerEntry : bookKeeperAdmin.readEntries(ledgerId, 0, lastEntry)) {
                ...
                log.debug().attr("entryId", lastEntry).attr("ledgerId", ledgerId).attr("cursor", cursorName)
                ...
                log.debug().attr("cursor", cursorName).attr("position", lastAckedMessagePosition)
                        .log("Cursor read position");
            }
        } catch (Exception e) {
            log.warn().attr("ledgerId", ledgerId).attr("cursor", cursorName).exception(e)
