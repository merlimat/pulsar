...
public class OwnedBundle {
     ...
     * @param pulsar
     * @param timeout
     *            timeout for unloading bundle. It doesn't throw exception if it timeout while waiting on closing all
     *            topics
     * @param timeoutUnit
     * @throws Exception
     */
    public CompletableFuture<Void> handleUnloadRequest(PulsarService pulsar, int timeout, TimeUnit timeoutUnit) {
        ...
        CompletableFuture<Void> future = pulsar.getNamespaceService().getOwnershipCache()
                ...
                .thenCompose(v -> pulsar.getBrokerService().unloadServiceUnit(bundle, timeout,timeoutUnit))
