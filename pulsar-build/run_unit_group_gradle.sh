...
function test_group_other() {
  ...
  gradle_test :pulsar-common:test \
    -PexcludedTestGroups='' \
    --tests 'org.apache.pulsar.common.util.DnsResolverTest'
}
...
function test_group_protobufv4() {
  echo "::group::Running Protobuf v4 tests"
  echo "::warning::Protobuf v4 tests are not yet available in the Gradle build (requires rebuild with different protobuf version)"
  echo "::endgroup::"
}
